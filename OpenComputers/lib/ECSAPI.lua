...
function ECSAPI.getOSApplication(application, downloadWallpapers)
    ...
    if application.type == "Application" then
		...
		if application.createShortcut then
			local desktopPath = "ServerOS/Desktop/"
			local dockPath = "ServerOS/System/OS/Dock/"
...
function ECSAPI.getAppsToUpdate(debug)
	...
	local pathToApplicationsFile = "ServerOS/System/OS/Applications.txt"
	local pathToSecondApplicationsFile = "ServerOS/System/OS/Applications2.txt"
...
function ECSAPI.newApplication(path, startName)
	...
	if ECSAPI.checkName(inputs[1] .. ".app", path) then
		...
		fs.copy("ServerOS/System/OS/Icons/SampleIcon.pic", name .. "Icon.pic")
...
function ECSAPI.newApplicationFromLuaFile(pathToLuaFile, pathWhereToCreateApplication)
	...
	data[2] = data[2] or "ServerOS/System/OS/Icons/SampleIcon.pic"
...
function ECSAPI.saveOSSettings()
	local pathToOSSettings = "ServerOS/System/OS/OSSettings.cfg"
...
local function OSIconsInit()
	...
	if not ECSAPI.OSIcons then
		...
		ECSAPI.OSIcons = {}
		ECSAPI.pathToIcons = "ServerOS/System/OS/Icons/"
...
function ECSAPI.launchIcon(path, arguments)
	...
	elseif fileFormat == ".pic" then
		shell.execute("ServerOS/Applications/Viewer.app/Viewer.lua open " .. path)
	--Если это 3D-модель
	elseif fileFormat == ".3dm" then
		shell.execute("ServerOS/Applications/3DPrint.app/3DPrint.lua open " .. path)
