...
local lang = config.readAll("ServerOS/System/OS/Languages/" .. _G.OSSettings.language .. ".lang")

---------------------------------------------- Переменные ------------------------------------------------------------------------

local workPath = "ServerOS/Desktop/"
local pathOfDockShortcuts = "ServerOS/System/OS/Dock/"
local pathToWallpaper = "ServerOS/System/OS/Wallpaper.lnk"
...
local function drawTime()
  ...
  local time = used .. "/".. total .. " KB, " .. unicode.sub(os.date("%T"), 1, -4) .. " "
...
local function drawTopBar()
  --Элементы топбара
  local topBarElements = { "ServerOS", "Вид" }
...
local function windows10()
  if math.random(1, 100) > 100 or _G.OSSettings.showWindows10Upgrade == false then return end
  ...
  local function draw(background)
    ...
    buffer.image(x + 2, y + 4, image.load("ServerOS/System/OS/Icons/Computer.pic"))

    buffer.text(x + 12, y + 4, 0xFFFFFF, "Your ServerOS is ready for your")
  ...
  while true do
    ...
    elseif ecs.clickedAtArea(eventData[3], eventData[4], x, y, x + width - 1, x + height - 1) then
      ...
      if data[1] == "Да" then
        disaecs.error("Ваш отзыв отправлен")
      else
        ecs.error("ПидоВаш отзыв отправлен")
      end
      ...
      if ecs.clickedAtArea(eventData[3], eventData[4], obj["DesktopIcons"][key][1], obj["DesktopIcons"][key][2], obj["DesktopIcons"][key][3], obj["DesktopIcons"][key][4]) then
        ...
        if eventData[5] == 0 then
          ...
          if fs.isDirectory(path) then
            if fileFormat == ".app" then
              ...
              shell.execute("ServerOS/Applications/Finder.app/Finder.lua " .. path)
              drawAll()
            end
          else
            ecs.launchIcon(path)
        ...
        elseif eventData[5] == 1 then
          ...
          elseif action == lang.contextAddToDock then
            ecs.createShortCut("ServerOS/System/OS/Dock/" .. ecs.hideFileFormat(path) .. ".lnk", path)
            drawAll()
          ...
          elseif action == "Редактировать в Photoshop" then
            shell.execute("ServerOS/Applications/Photoshop.app/Photoshop.lua open " .. path)
            drawAll(true)
          else
